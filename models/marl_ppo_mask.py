...
from common.maskable_buffers import MARLMaskableDictRolloutBuffer
from sb3_contrib.common.maskable.policies import MaskableActorCriticPolicy
from sb3_contrib.common.maskable.utils import get_action_masks, is_masking_supported
from sb3_contrib.ppo_mask.policies import CnnPolicy, MlpPolicy, MultiInputPolicy

SelfMaskablePPO = TypeVar("SelfMARLMaskablePPO", bound="MARLMaskablePPO")


class MARLMaskablePPO(OnPolicyAlgorithm):
    """
    ...
    rollout_buffer: MARLMaskableDictRolloutBuffer  # type: ignore[assignment]
    ...
    def __init__(
        ...
        rollout_buffer_class: Optional[Type[MARLMaskableDictRolloutBuffer]] = None,
    ...
    def _setup_model(self) -> None:
        ...
        if self.rollout_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                self.rollout_buffer_class = MARLMaskableDictRolloutBuffer
            else:
    ...
    def collect_rollouts(
        ...
        assert isinstance(
            rollout_buffer, MARLMaskableDictRolloutBuffer
        ...
        n_steps = 0
        action_masks = None
        agent_made_first_move = np.zeros((env.num_envs, self.n_agents), dtype=bool)
        ...
        while n_steps < n_rollout_steps:
            ...
            new_obs, rewards, all_agent_rewards, dones, infos = env.step(actions)
            ...
            rollout_buffer.add(
                ...
                agent_id=self._last_obs['current_agent'],
            )
            ...
            for env_idx, done in enumerate(dones):
                if (
                    ...
                    # print(rewards)
                    # print(all_agent_rewards)
                    # import sys
                    # sys.exit()
                    # back assign rewards
                    # terminal_obs = self.policy.obs_to_tensor(infos[env_idx]["terminal_observation"])[0]
                    # with th.no_grad():
                    #     terminal_value = self.policy.predict_values(terminal_obs)[0]
                    # rewards[env_idx] += terminal_value
                    rollout_buffer.back_assign_agent_rewards(env_idx, all_agent_rewards[env_idx])
            self._last_obs = new_obs  # type: ignore[assignment]
            self._last_episode_starts = dones | ~agent_made_first_move[np.arange(env.num_envs), new_obs['current_agent']]  # n_envs
            agent_made_first_move[dones, :] = False
            agent_made_first_move[np.arange(env.num_envs), new_obs['current_agent']] = True
        win_rate = rollout_buffer.get_win_rate()
        for i, rate in enumerate(win_rate):
            self.logger.record(f"Agent {i} win rate", rate)
